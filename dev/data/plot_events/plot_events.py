...
def main():
    ...
    with h5py.File(args.events) as handle:
        ...
        long_noisy = (
            handle["long_noisy"][:] if "long_noisy" in handle else None
        )
        long_clean = (
            handle["long_clean"][:] if "long_clean" in handle else None
        )
    ...
    if glitch_file:
        name = (
            glitch_file.decode()
            if isinstance(glitch_file, bytes)
            else (glitch_file)
        )
    ...
    fig, axes = plt.subplots(
        n_ifos, 2, figsize=(14, 4.4 * n_ifos), squeeze=False
    )
