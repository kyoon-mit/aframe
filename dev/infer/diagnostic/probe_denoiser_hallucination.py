...
def load_model(ckpt, config_path, device):
    ...
    arch_cls = getattr(__import__(module_path, fromlist=[class_name]),
                       class_name)
...
def draw_background_row(ax_time, ax_freq, noisy, predicted, t, sample_rate):
    ...
    ax_time.plot(
        t, predicted, lw=0.9, color=COLOR_PRED_BG, label="prediction"
    )
    ax_time.set_title(
        f"prediction RMS = {rms(predicted):.3e}"
        f"   (input {rms(noisy):.3e})",
        fontsize=8,
        loc="right",
    )
...
def draw_injected_row(
    ax_time, ax_freq, noisy, predicted, target, sample_rate
):
