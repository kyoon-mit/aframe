...
class MetricsRateLimiter:
    ...
    def __init__(
        ...
        Args:
            metrics_url:
                URL to the Triton metrics endpoint (e.g., "http://host:8002/metrics")
...
def build_metrics_url(address: str, port: int = 8002) -> str:
    """
    Build the Triton metrics URL from the inference server address.

    Args:
        address: Triton server address (e.g., "host:8001")
        port: Port for the metrics endpoint (default 8002)

    Returns:
        Metrics URL (e.g., "http://host:8002/metrics")
    """
    if "://" in address:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            triton_address:
                Address of the Triton inference server (e.g., "host:8001").
                Used to build the metrics URL for rate limiting.
            metrics_port:
                Port for the Triton metrics endpoint (default 8002)
            max_pending_requests:
