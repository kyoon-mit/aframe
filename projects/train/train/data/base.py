...
class BaseAframeDataset(pl.LightningDataModule):
    ...
    Args:
        ...
        windowing:
            A `WindowConfig` object specifying the kernel length and the
            valid placement range of the window relative to the merger.
            See `WindowConfig` for details.
        fduration:
    ...
    def sample_length(self) -> float:
        ...
        return (
            self.window_config.kernel_length
    ...
    def window_config(self) -> WindowConfig:
        """
        ``windowing`` as a ``WindowConfig``. When the datamodule is built by
        the Lightning CLI the hparam arrives as a plain dict, so reconstruct
        the dataclass to expose its computed ``left_pad``/``right_pad``.
        """
        windowing = self.hparams.windowing
        if isinstance(windowing, WindowConfig):
            return windowing
        return WindowConfig(**windowing)
    ...
    def left_pad_size(self) -> int:
        ...
        """
        return (
            int(self.window_config.left_pad * self.hparams.sample_rate)
            + self.filter_size // 2
        )
    ...
    def right_pad_size(self) -> int:
        ...
        """
        return (
            int(self.window_config.right_pad * self.hparams.sample_rate)
            + self.filter_size // 2
        )
    ...
    def slice_waveforms(self, waveforms: torch.Tensor) -> torch.Tensor:
        ...
        kernel_size = (
            int(self.window_config.kernel_length * self.hparams.sample_rate)
            + self.filter_size
        )
    ...
    def build_transforms(self):
        ...
        """
        window_length = (
            self.window_config.kernel_length + self.hparams.fduration
        )
    ...
    def setup(self, stage: str) -> None:
        ...
        self._logger.info(str(self.window_config))
