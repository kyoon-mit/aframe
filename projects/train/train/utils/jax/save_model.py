...
class JAXCheckpointManager(Callback):
    ...
    def setup(
        ...
        if wandb_logger is not None:
            # Save under the per-run wandb directory so checkpoints are easy
            # to find, e.g. <save_dir>/<project>/<wandb_id>/checkpoints
            save_dir = Path(wandb_logger.save_dir)
            self.output_dir = (
                save_dir
                / wandb_logger.name
    ...
    @staticmethod
    def _get_wandb_logger(trainer: Trainer) -> WandbLogger | None:
