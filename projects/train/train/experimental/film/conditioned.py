...
class ConditionedDenoiser(Denoiser):
    ...
    The parameters named in ``cond_params`` are read from the batch, log
    transformed, and passed to the architecture as a conditioning vector.
    Training batches are passed without conditioning with probability
    ``cond_dropout``; validation is never conditioned. Rows with no
    injection carry NaN parameters and are conditioned on zeros.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.save_hyperparameters("cond_params", "cond_dropout")
    ...
    def _build_cond(self, params: dict, device) -> Optional[torch.Tensor]:
        ...
        cond = torch.stack(
            [
                params[name].to(device).float().log()
                for name in self.cond_params
            ],
            dim=-1,
        )
